/* ─────────────────────────────────────────────────────────────────────────────
 * FILE   src/mod/batch_mod_ctx.cu
 * ROLE   core of the batched modular context, reduction-independent
 *
 * HOW    Construction and teardown, conversion to and from the working
 *        form, the pass/fail check, and the modmul/modsq drivers
 *        (polynomial product, then reduce_batch). The reduction itself
 *        lives in reductions/, the report in perf/mod_perf.cu.
 *
 * CHANGELOG
 *   2026-08-11  to_residue_batch keeps N on the host and skips GMP entirely
 *               when the working form is the plain residue and the value is
 *               already reduced. It used to pull N back over PCIe and run
 *               three O(n^2) conversions per candidate, per call.
 * ───────────────────────────────────────────────────────────────────────────── */
...
__global__ static void check_passed_kernel(
    ...
    const T *__restrict__ ref_a,
    const T *__restrict__ ref_b,
...
// Barrett's working form is the plain residue, so the conversion is x mod N.
// Every call site feeds a value that is already < N (1, N-1, a small witness),
// which makes the conversion the identity — detect that with an O(n) limb
// compare and skip GMP entirely. Montgomery's form is x·R mod N: never a copy.
#if MOD_REDUCTION_ALG == MOD_RED_BARRETT
static constexpr bool RESIDUE_IS_PLAIN = true;
#else
static constexpr bool RESIDUE_IS_PLAIN = false;
#endif
                                   ...
                                   std::vector<uint64_t> &out_all) const
{
    ...
    for (int i = 0; i < n_batch; i++)
    {
        const uint64_t *xi = x_all.data() + (size_t)i * n_limbs;
        const uint64_t *Ni = N_host.data() + (size_t)i * n_limbs;
        uint64_t *oi = out_all.data() + (size_t)i * n_limbs;

        if (RESIDUE_IS_PLAIN && limbs_cmp(xi, Ni, n_limbs) < 0)
        {
            std::copy(xi, xi + n_limbs, oi);
            continue;
        }
        limbs_to_mpz(xm, xi, n_limbs);
        limbs_to_mpz(N, Ni, n_limbs);
        mod_residue_forward(res, xm, N, n_limbs);
        mpz_to_limbs(oi, n_limbs, res);
    }
    mpz_clear(xm);
    mpz_clear(N);
    mpz_clear(res);
...
void BatchModCtx::from_residue_batch(const LimbT *d_x, std::vector<uint64_t> &out_all) const
{
    ...
    for (int i = 0; i < n_batch; i++)
    {
        const uint64_t *xi = x_h.data() + (size_t)i * n_limbs;
        const uint64_t *Ni = N_host.data() + (size_t)i * n_limbs;
        uint64_t *oi = out_all.data() + (size_t)i * n_limbs;
