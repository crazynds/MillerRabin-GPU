/* ─────────────────────────────────────────────────────────────────────────────
 * FILE   src/mr/miller_rabin_runner.cu
 * ROLE   global witness sweep over all candidates
 *
 * HOW    Takes every candidate at once. For each witness: test all
 *        survivors in sub-batches of MR_BATCH_SIZE, then compact globally
 *        before moving to the next witness.
 *
 * NOTE   Witness-major rather than candidate-major on purpose: witness 2
 *        then only ever sees what survived witness 1, and most composites
