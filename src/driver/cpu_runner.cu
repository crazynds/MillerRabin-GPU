/* ─────────────────────────────────────────────────────────────────────────────
 * FILE   src/driver/cpu_runner.cu
 * ROLE   CPU reference path (--cpu, --cpu-parallel, --threads N)
 *
 * HOW    Runs the same group semantics as the GPU driver but tests each
 *        candidate with GMP mpz_probab_prime_p, one candidate at a time,
 *        spread over N threads.
 *
 * NOTE   Exists to cross-check results and to have an honest baseline to
 *        compare against; it is not the production path.
 * ───────────────────────────────────────────────────────────────────────────── */
