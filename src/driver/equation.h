/* ─────────────────────────────────────────────────────────────────────────────
 * FILE   src/driver/equation.h
 * ROLE   evaluates an integer equation string into an mpz_t
 *
 * HOW    Recursive-descent parser with standard precedence and right-
 *        associative ^: expr := term (("+"|"-") term)*, term := factor
 *        (("*"|"/"|"%") factor)*, power := unary ("^" power)?. Every
 *        literal is arbitrary precision, so 10^100000 is written literally
